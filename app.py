...
# ------------------------------------------------------------
# Google Places API 検索 (カテゴリ厳密分け・不要施設除外)
# ------------------------------------------------------------
def search_places_google(location_str, radius_km, search_query_keyword):
    ...
    # スイーツ系・観光系・ご飯系を明確に判定して適切なタイプを設定
    if any(w in keyword_lower for w in ["アイス", "クレープ", "アサイー", "ケーキ", "パフェ", "パン", "ベーカリー", "ジェラート", "スイーツ", "喫茶店"]):
        target_types_list = [["cafe"], ["bakery"], ["ice_cream_shop"], ["confectionery"]]
    ...
    # スイーツ検索時に絶対に除外したいチェーン店やファストフードのキーワード
    unwanted_name_keywords = ["ホテル", "hotel", "旅館", "宿"]
    if any(w in keyword_lower for w in ["アイス", "クレープ", "ケーキ", "パフェ", "スイーツ", "ジェラート"]):
        unwanted_name_keywords.extend(["マクドナルド", "mcdonald", "すき家", "吉野家", "松屋", "サイゼリヤ", "ガスト"])
    ...
    for p in all_raw_places:
        ...
        if any(x in name_check.lower() for x in unwanted_name_keywords):
            continue
